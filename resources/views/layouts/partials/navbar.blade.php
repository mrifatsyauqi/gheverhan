{{-- Announcement Bar --}}
@if(\App\Models\StoreSetting::get('announcement_bar_active', '0') == '1')
<div class="bg-primary text-white text-xs sm:text-sm font-medium py-2 px-4 text-center">
    {{ \App\Models\StoreSetting::get('announcement_bar_text', 'Gratis Ongkir Seluruh Indonesia!') }}
</div>
@endif
        ...
        <div class="flex items-center justify-between h-16">
            ...
            <a href="{{ route('home') }}" class="flex-shrink-0 flex items-center gap-2">
                @if(\App\Models\StoreSetting::get('store_logo'))
                    <img src="{{ \App\Models\StoreSetting::get('store_logo') }}" alt="Logo" class="h-8">
                @else
                    <span class="text-xl font-extrabold tracking-tight text-primary">{{ \App\Models\StoreSetting::get('store_name', 'GHEVERHAN') }}</span>
                @endif
            </a>
                ...
                <div x-data="{ open: false }" class="relative">
                    ...
                    <div x-show="open" @click.away="open = false"
                        ...
                        @if(Auth::user()->isAdmin())
                            <a href="{{ route('admin.dashboard') }}" class="block px-4 py-2 text-sm font-bold text-primary hover:bg-surface transition-colors border-b border-gray-100">Dashboard Admin &rarr;</a>
                        @endif
